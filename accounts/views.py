...
from accounts.tasks import (resend_verification_otp, send_verification_email,
                            send_verification_otp)
from client.models import Wallet
from management.models import SubscriptionPack, Total_pack, UserSubscription
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            verification_link = request.build_absolute_uri(
                f"/verify_email/{user.email_verification_token}/"
            )
            ...
            request.session["register_email"] = (
                email  # It is used to resent the email , also it is used in the adpaters.
            )
            user = form.save()
    ...
    if user:
        ...
        if token:
            ...
            return render(
                ...
                {
                    "email": email,
                    "error": "The Email is verified, You can sign in to Your account.",
                },
            )
...
def verify_email(request, token):
    ...
    if user:
        ...
        request.session["email"] = (
            user.email
        )  # this session is created for to use in the upcoming procedures
        ...
        if email:
            ...
            if role == "freelancer":
                UserSubscription.objects.create(
                    user=user, subscription_pack=subscriptionPack
                )
                Total_pack.objects.create(
                    user=user,
                    gig_count=subscriptionPack.max_gigs,
                    connection_limit=subscriptionPack.connection_limit,
                )
        ...
        if user:
            ...
            if user.is_verified:
                ...
                return render(
                    ...
                    "accounts/login.html",
                    {
                        "error": "Email is not verified.You can verify email by simply clicking the link that we sent on your Email"
                    },
                )
        else:
            return render(
                ...
                "accounts/login.html",
                {
                    "error": "Invalid email or password.<br>Signed up with Google? Use Google to log in.<br>Account not verified earlier? Please sign up again."
                },
            )
    else:
        ...
        if user:
            ...
            if request.session.get("otp_sent"):
                return render(
                    ...
                    {"message": "Check Your Email,Otp Has been sent."},
                )
            ...
            return render(
                request, "accounts/forgot_password.html", {"error": "Invalide Email"}
            )
