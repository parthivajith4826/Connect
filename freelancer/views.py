...
from client.models import Card, Categories, User
from management.models import (SubscriptionPack, SubscriptionTransaction,
                               Total_pack, UserSubscription)

from .forms import ContactForm, CreategigForm, ProfileForm
...
def home(request):
    ...
    if not user.profile_completed:
        ...
        connections_accepted = Connections.objects.filter(
            user=user, status="accepted"
        ).count()
        connections_pending = Connections.objects.filter(
            user=user, status="pending"
        ).count()
        connections_rejected = Connections.objects.filter(
...
def profile(request):
    ...
    if request.method == "POST":
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            return render(
                request, "freelancer/profile.html", {"form1": form1, "form2": form2}
            )
    ...
    return render(
        ...
        {"form1": form1, "form2": form2, "profile": profile},
    )
...
def add_gig(request):
    ...
    if usersubscription:
        ...
        if (
            not total_pack.gig_count == 0
        ):  # checking the gig_count,gig creation is only possible if it is greater than 0
            ...
            if request.method == "POST":
                ...
                if images:
                    if len(images) > 3:
                        form.add_error(
                            None, "You can upload a maximum of 3 images only."
                        )
                        is_form_valid = False
                ...
                if is_form_valid:
                    ...
                    for img in images:
                        GigImages.objects.create(gig_id=gig, image=img)
            ...
            return render(
                request, "freelancer/home.html", {"gigs": gigs, "error": error}
            )
...
def edit_gig(request, slug):
    ...
    return render(
        request,
        "freelancer/edit_gig.html",
        {"form": form, "categories": categories, "gig": gig, "images": images_db},
    )
...
def subscriptions(request):
    ...
    return render(
        request,
        "freelancer/subscriptions.html",
        {"subscriptions": subscriptions, "freeplan": freeplan},
    )
    ...
    if event_type == "payment_intent.succeeded":
        ...
        with transaction.atomic():
            ...
            total_pack, _ = Total_pack.objects.update_or_create(
                user=user,
                defaults={"gig_count": gig_count, "connection_limit": connection_limit},
            )
...
def find_work(request):
    ...
    if minimum_price is not None and maximum_price is not None:
        cards = cards.filter(
            min_budget__gte=minimum_price, max_budget__lte=maximum_price
        )
    ...
    return render(
        request, "freelancer/find_work.html", {"cards": cards, "categories": categories}
    )
...
def card_details(request, slug):
    ...
    return render(
        request,
        "freelancer/card_details.html",
        {"card": card, "skill_list": skill_list},
    )
...
def show_gigs(request, card_slug=None):
    ...
    usersubscription = UserSubscription.objects.filter(
        user=request.user, is_active=True
    )
    if not usersubscription:
        messages.error(
            request, "No active subscriptions. Unable to establish a connection."
        )
    ...
    if total_pack.connection_limit == 0:
        messages.error(
            request,
            "Connection limit reached (0). No new connections can be established.",
        )
    return render(
        request,
        "freelancer/show-gigs.html",
        {"gigs": gigs, "gig_count": gig_count, "card": card, "total_pack": total_pack},
    )
...
def show_gig_details(request, slug):
    ...
    return render(
        request,
        "freelancer/show_gig_details.html",
        {"gig": gig, "skill_list": skill_list},
    )
...
def create_connection(request, card_slug):
    ...
    with transaction.atomic():
        connection, created = Connections.objects.get_or_create(
            user=request.user, client_user=client_user, card=card, gig=gig
        )
    ...
    if not created:
        ...
        if connection.status == "pending":
            ...
            return render(
                request,
                "freelancer/show-gigs.html",
                {"gigs": gigs, "gig_count": gig_count, "card": card},
            )
...
def subscription_details(request):
    ...
    return render(
        request,
        "freelancer/subscription_status.html",
        {"subscriptions": subscriptions, "total_pack": total_pack},
    )
...
def my_connections(request):
    ...
    return render(
        request,
        "freelancer/my_connections.html",
        {"connections": connections, "connections_count": connections_count},
    )
...
def gig_preview(request, slug):
    ...
    skills = skills.split(",")
    return render(
        request, "freelancer/gig_preview.html", {"gig": gig, "skills": skills}
    )
