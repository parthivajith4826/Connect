...
from .forms import CreatecardForm, LocationForm, ProfileForm
...
def home(request):
    ...
    card = (
        Card.objects.filter(client_id=user)
...
def profile(request):
    ...
    if request.method == "POST":
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            return render(
                request, "client/profile.html", {"form1": form1, "form2": form2}
            )
        ...
        return render(
            request,
            "client/profile.html",
            {"user": user, "form1": form1, "form2": form2},
        )
        ...
        if is_form_valid:
            with transaction.atomic():
                ...
                WalletTransactions.objects.create(
                    wallet=wallet,
                    amount=pricing.card_creation_price,
...
def view_card(request, slug):
    ...
    if card:
        ...
        return render(
            request,
            "client/view.html",
            {"card": card, "images": images, "skill_list": skill_list},
        )
    else:
...
def edit_card(request, slug):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "client/edit_card.html",
            {"form": form, "categories": categories, "card": card, "images": images_db},
        )
...
def add_fund(request):
    if request.method == "POST":
        ...
        try:
            ...
            return render(request, "client/add_funds.html", {"error": "Invalid amount"})

        if amount <= 0:
            return render(
                request,
                "client/add_funds.html",
                {"error": "Amount must be greater than zero"},
            )

        if amount < 50:
            return render(
                request, "client/add_funds.html", {"error": "Minimum amount: ₹50.00"}
            )
        ...
        return render(
            request,
            "client/wallet_pay.html",
            {
                "client_secret": intent.client_secret,
                "STRIPE_PUBLIC_KEY": settings.STRIPE_PUBLIC_KEY,
                "amount": amount,
                "success_url": request.build_absolute_uri("/client/wallet/result/"),
            },
        )
...
def stripe_webhook(request):
    ...
    try:
        event = stripe.Webhook.construct_event(
            payload, sig_header, settings.STRIPE_WALLET_WEBHOOK_SECRET
        )
...
def withdraw(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render(request, "client/withdraw.html", {"error": "Invalid amount"})

        if amount <= 0:
            return render(
                request,
                "client/withdraw.html",
                {"error": "Amount must be greater than zero"},
            )
...
def wallet_result(request):
    ...
    txn = WalletTransactions.objects.filter(stripe_payment_intent=intent_id).first()
...
def gig_details(request, gig_slug, card_slug):
    ...
    if total_rating > 0:
        average_rating = (
            total_rating / Rating.objects.filter(reviewee=gig.freelancer_id).count()
        )
    ...
    if request.method == "POST":
        ...
        if action == "accept":
            ...
            with transaction.atomic():
                ...
                messages.success(
                    request,
                    "🎉 Connection established!  You’re now connected, and contact details are ready to view.",
                )
...
def review(request):
    ...
    try:
        ...
        rating_old = Rating.objects.filter(
            reviewer=request.user, gig=gig, card=card
        ).first()
